...
void search(vector <string> lines, string singleword, int start, int end) {
...
int main(void)
{
	...
	for (int i = 0; i < wordslist.size(); i++) {
		search(lines, wordslist[i], 0, lines.size());
	}
	auto finishtread = std::chrono::high_resolution_clock::now();
	std::chrono::duration<double> timethread = finishtread - startthread;
	cout << " time one thread : " << timethread.count() * 1000000 << " Microsecond\n";
